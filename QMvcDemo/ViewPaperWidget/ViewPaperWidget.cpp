...
{
    g_Widget->geometryFit(0, 302, 800, 178, this);
}
...
void ViewPaperWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (d->m_Finish) {
        ...
        int distance = 160 *  g_Widget->widthScalabilityFactor();
...
void ViewPaperWidget::onWidgetTypeChange(const Widget::Type destinationType, const Widget::Type requestType, const QString &status)
{
    ...
    case Widget::T_ViewPaper2: {
        if (WidgetStatus::RequestShow == status) {
            ...
            g_Widget->setWidgetType(Widget::T_ViewPaper2, Widget::T_Undefine, WidgetStatus::Show);
        } else if (WidgetStatus::Show == status) {
            if ((1600 * g_Widget->widthScalabilityFactor()) != horizontalOffset()) {
                horizontalScrollBar()->setValue(1600 * g_Widget->widthScalabilityFactor());
            }
            Q_D(ViewPaperWidget);
            d->m_CurrentViewPapger = 2;
            d->m_Finish = true;
...
void ViewPaperWidget::onFinished()
{
    ...
    case 2: {
        g_Widget->setWidgetType(Widget::T_ViewPaper2, Widget::T_ViewPaper1, WidgetStatus::RequestShow);
        break;
    }
    default: {
        break;
...
void ViewPaperWidgetPrivate::initializeBasic()
{
    ...
    data.m_Type = Widget::T_airconditioner;
    ...
    data.m_Type = Widget::T_VoiceAssistant;
    data.m_Title = QString("Navi");
    data.m_Normal = QString(":/Images/ViewPaperWidgetSettingNormal.png");
    data.m_Press = QString(":/Images/ViewPaperWidgetSettingPress.png");
    m_MapType.insert(m_MapType.size(), data);
...
void ViewPaperWidgetPrivate::reload()
{
    ...
    thirdItem->setSizeHint(QSize(800 * g_Widget->widthScalabilityFactor(), 178 * g_Widget->heightScalabilityFactor()));
...
void ViewPaperWidgetPrivate::initializeFirstView()
{
    ...
    if (flag) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            m_FirstViewPaperVariant.m_IconRectList.append(g_Widget->geometryFit(step + (step + 106) * i, 37, 106, 106));
            m_FirstViewPaperVariant.m_TextRectList.append(g_Widget->geometryFit(step + (step + 106) * i, 178 - 40, 106, 40));
    ...
    if (flag) {
        ...
        if (count > 0) {
            for (int i = 0; i < count; ++i) {
                ...
                m_SecondViewPaperVariant.m_IconRectList.append(g_Widget->geometryFit(45 + (45 + 106) * i, 37, 106, 106));
                m_SecondViewPaperVariant.m_TextRectList.append(g_Widget->geometryFit(45 + (45 + 106) * i, 178 - 40, 106, 40));
    ...
    if (flag) {
        ...
        if (count > 0) {
            for (int i = 0; i < count; ++i) {
                ...
                m_ThirdViewPaperVariant.m_IconRectList.append(g_Widget->geometryFit(45 + (45 + 106) * i, 37, 106, 106));
                m_ThirdViewPaperVariant.m_TextRectList.append(g_Widget->geometryFit(45 + (45 + 106) * i, 178 - 40, 106, 40));
...
{
    g_Widget->setWidgetType(m_MapType[index].m_Type, Widget::T_ViewPaper0, WidgetStatus::RequestShow);
    if ((m_MapType[index].m_Type == Widget::T_Link) && (CLT_HICAR == SettingPersistent::getCarLinkType())) {
        g_Link->reqeustHiCarBroadcastStatus(true);
    }
}

void ViewPaperWidgetPrivate::secondViewPaperHandler(const unsigned int index)
{
    g_Widget->setWidgetType(m_MapType[index + 5].m_Type, Widget::T_ViewPaper1, WidgetStatus::RequestShow);
}

void ViewPaperWidgetPrivate::thirdViewPaperHandler(const unsigned int index)
{
    g_Widget->setWidgetType(m_MapType[index + 10].m_Type, Widget::T_ViewPaper2, WidgetStatus::RequestShow);
}
