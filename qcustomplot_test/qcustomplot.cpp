...
void QCPAxis::scaleRange(double factor, double center)
{
  ...
  if (mScaleType == stLinear)
  {
    ...
    {
        //qDebug() << "mScaleType == stLinear;QCPRange::validRange(newRange)";
  ...
  } else // mScaleType == stLogarithmic
  {
    if ((mRange.upper < 0 && center < 0) || (mRange.upper > 0 && center > 0)) // make sure center has same sign as range
    {
      ...
      if (QCPRange::validRange(newRange))
      {
          //qDebug() << "mScaleType == stLogarithmic;QCPRange::validRange(newRange)";
...
{
    //qDebug() << "QCustomPlot::mousePressEvent(QMouseEvent *event)";
...
bool QCustomPlot::event(QEvent *event)
{
    ...
    {
        //qDebug() << "QCustomPlot TouchBegin";
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
        QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
        const QTouchEvent::TouchPoint touchPoint0 = touchPoints.first();
        return layoutElementAt(touchPoint0.pos())->event(event);
        break;
    }
    case QEvent::TouchUpdate:
    {
        //qDebug() << "QCustomPlot TouchUpdate";
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
        QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
        if (touchPoints.count() == 2) {
            return layoutElementAt(mMousePressPos)->event(event);
        }
        break;
    }
    case QEvent::TouchEnd:
    {
        //qDebug() << "QCustomPlot TouchEnd";
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
        QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
...
void QCPAxisRect::mousePressEvent(QMouseEvent *event)
{
  ...
  {
    //qDebug() << "QCPAxisRect::mousePressEvent";
    mDragging = true;
    ...
    if (mParentPlot->interactions().testFlag(QCP::iRangeDrag))
    {
        if (mRangeDragHorzAxis)
        {
            ...
            //qDebug() << "mDragStartHorzRange" << mDragStartHorzRange.minRange << mDragStartHorzRange.maxRange;
        }
...
void QCPAxisRect::mouseMoveEvent(QMouseEvent *event)
{
  ...
  {
      //qDebug() << "QCPAxisRect::mouseMoveEvent";
...
{
  Q_UNUSED(event)
    //qDebug() << "QCPAxisRect::mouseReleaseEvent";
  mDragging = false;
  if (mParentPlot->noAntialiasingOnDrag())
  {
...
bool QCPAxisRect::event(QEvent *event)
{
    ...
    case QEvent::TouchBegin:
    {
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
        QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
        //qDebug() << "QCPAxisRect QEvent::TouchBegin" << touchPoints.count() << touchPoints.size();
        //if(touchPoints.count() == 2)
        //{
            mTouchDistance = 0;
            mbTouch = true;
    ...
    case QEvent::TouchUpdate:
    {
        ...
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
        QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
        //qDebug() << "QEvent::TouchUpdate touchPoints.count() = " << touchPoints.count();
        if (touchPoints.count() == 2) {
            qreal u_x = touchPoints.at(1).pos().x() - touchPoints.at(0).pos().x();
            qreal u_distance = qSqrt(u_x*u_x);
            if(mTouchDistance == 0)
            {
                mTouchDistance = u_distance;
                return true;
            }
            qreal u_offset = u_distance - mTouchDistance;
            mTouchDistance = u_distance;
            //qDebug() << "u_offset =" << u_offset;
            ...
            if (mParentPlot->interactions().testFlag(QCP::iRangeZoom))
            {
              ...
              {
                double factor;
                double wheelSteps = u_offset/120.0; // a single step delta is +/-120 usually
                if (mRangeZoom.testFlag(Qt::Horizontal))
                {
                  factor = pow(mRangeZoomFactorHorz, wheelSteps);
                  ...
                  if (mRangeZoomHorzAxis.data())
                  {
                      double dCenter = (touchPoints.at(0).pos().x() + touchPoints.at(1).pos().x())/2;

                      mRangeZoomHorzAxis.data()->scaleRange(factor, mRangeZoomHorzAxis.data()->pixelToCoord(dCenter));
                      //qDebug() << "scaleRange " << dCenter << factor << mRangeZoomHorzAxis.data()->pixelToCoord(dCenter);
                      if (mRangeDragHorzAxis)
                      {
                          mDragStartHorzRange = mRangeDragHorzAxis.data()->range();
                          //qDebug() << "mDragStartHorzRange" << mDragStartHorzRange.minRange << mDragStartHorzRange.maxRange;
                ...
                if (mRangeZoom.testFlag(Qt::Vertical))
                {
                    ...
                    {
                        double dCenter = (touchPoints.at(0).pos().x() + touchPoints.at(1).pos().x())/2;
                        mRangeZoomVertAxis.data()->scaleRange(factor, mRangeZoomVertAxis.data()->pixelToCoord(dCenter));
                    }
                }
                mParentPlot->replot();
              }
    ...
    case QEvent::TouchEnd:
    {
        //qDebug() << "QCPAxisRect QEvent::TouchEnd";
        mTouchDistance = 0;
        mbTouch = false;
        break;
    }
